...
static LADSPA_Handle instantiateVynil(
 ...
 unsigned long s_rate) {
	...
	buffer_m = (LADSPA_Data*) malloc(sizeof(LADSPA_Data) * buffer_size);
	buffer_s = (LADSPA_Data*) malloc(sizeof(LADSPA_Data) * buffer_size);
	buffer_mask = buffer_size - 1;
	buffer_pos = 0;
	click_gain = 0;
	phi = 0.0f; /* Angular phase */

	click_buffer = (LADSPA_Data*) malloc(sizeof(LADSPA_Data) * CLICK_BUF_SIZE);
	...
	lowp_m = (biquad*) calloc(sizeof(biquad), 1);
	lowp_s = (biquad*) calloc(sizeof(biquad), 1);
	highp = (biquad*) calloc(sizeof(biquad), 1);
	noise_filt = (biquad*) calloc(sizeof(biquad), 1);
