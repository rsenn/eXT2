...
static LADSPA_Handle instantiateGsm(
 ...
 unsigned long s_rate) {
	...
	src = (gsm_signal*) malloc(sizeof(gsm_signal) * 160);
	dst = (gsm_signal*) malloc(sizeof(gsm_signal) * 163);
	dry = (LADSPA_Data*) malloc(sizeof(LADSPA_Data) * 160 * resamp);
	handle = NULL;

	blf = (biquad*) malloc(sizeof(biquad));
	biquad_init(blf);
