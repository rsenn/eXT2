...
blo_h_tables *blo_h_tables_new(int table_size)
{
	...
	This = (blo_h_tables*) malloc(sizeof(blo_h_tables));
	...
	if ((shm_fd = shm_open(shm_path, O_RDONLY, 0)) > 0) {
		...
		all_tables = (float*) mmap(0, all_tables_size, PROT_READ, MAP_SHARED,
				shm_fd, 0);
		close(shm_fd);
		...
		all_tables = (float*) mmap(0, all_tables_size, PROT_READ | PROT_WRITE,
				MAP_SHARED, shm_fd, 0);
		close(shm_fd);
	}
	...
	if (!all_tables) {
		all_tables = (float*) malloc(all_tables_size);
...
{
	blo_h_osc *This = (blo_h_osc*) malloc(sizeof(blo_h_osc));
