...
static LADSPA_Handle instantiateImp(
 ...
 unsigned long s_rate) {
	...
	impulse_freq = (fftw_real **) local_malloc(IMPULSES * sizeof(fftw_real *));
	for (i=0; i<IMPULSES; i++) {
	  impulse_freq[i] = (fftw_real *) local_malloc(MAX_FFT_LENGTH * sizeof(fftw_real));
	}

	block_time = (fftw_real *) local_malloc(MAX_FFT_LENGTH * sizeof(fftw_real));
	block_freq = (fftw_real *) local_malloc(MAX_FFT_LENGTH * sizeof(fftw_real));
	op = (fftw_real *) local_malloc(MAX_FFT_LENGTH * sizeof(fftw_real));
	overlap = (LADSPA_Data*) local_malloc(MAX_FFT_LENGTH * sizeof(LADSPA_Data));
	opc = (LADSPA_Data*) local_malloc(SEG_LENGTH * sizeof(LADSPA_Data));
