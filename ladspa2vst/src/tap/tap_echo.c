...
activate_Echo(LADSPA_Handle Instance) {
	...
	if ((ptr->ringbuffer_L = (LADSPA_Data *)
	     calloc(MAX_DELAY * ptr->sample_rate / 1000,
		    sizeof(LADSPA_Data))) == NULL)
		exit(1);
	if ((ptr->ringbuffer_R = (LADSPA_Data *)
	     calloc(MAX_DELAY * ptr->sample_rate / 1000,
		    sizeof(LADSPA_Data))) == NULL)
		exit(1);
	if ((ptr->buffer_pos_L = (unsigned long *)calloc(1, sizeof(unsigned long))) == NULL)
		exit(1);
	if ((ptr->buffer_pos_R = (unsigned long *)calloc(1, sizeof(unsigned long))) == NULL)
		exit(1);
