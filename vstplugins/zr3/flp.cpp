...
void flp::setProgram (long program)
{
//	this->setProgram(program,false);
	this->setProgram(program,true);
}

void flp::setProgram (long program,bool force)
{
	long x;
...
void flp::getParameterLabel (long index, char *label)
{
	// forget it, we don't fully support non-GUI interface...
	sprintf(label, "",index);
}

void flp::getParameterDisplay (long index, char *text)
{
	text[0] = 0;
	switch(index)
	{
...
void flp::getParameterName (long index, char *label)
{
	if(index<=LAST_PARAM)
...
void flp::my_setParameter (long index, float value)
{
    ...
    std::cout << "setParameter " << index << " to " << value << std::endl;
...
void flp::setParameter (long index, float value)
{
/*
...
float flp::getParameter (long index)
{
	gp_value=0;
...
bool flp::getOutputProperties (long index, VstPinProperties* properties)
{
...
bool flp::getProgramNameIndexed (long category, long index, char* text)
{
...
bool flp::copyProgram (long destination)
{
