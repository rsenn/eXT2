...
#if MOTIF
#include "images/cknob.xpm"
#include "images/dbblack.xpm"
#include "images/dbbrown.xpm"
#include "images/dbwhite.xpm"
#include "images/inside.xpm"
#include "images/miniledred.xpm"
#include "images/minioffon.xpm"
#include "images/yellow.xpm"
#include "images/onoffgreen.xpm"
#include "images/panelfx.xpm"
#include "images/panelvoice.xpm"
#include "images/vonoff.xpm"
#include "images/vu.xpm"
...
CResTable xpmResources = {
    ...
    {IDB_dbwhite,       dbwhite},
    {IDB_inside,        inside},
    {IDB_miniledred,    miniledred},
    {IDB_minioffon,     minioffon},
    {IDB_yellow,        yellow},
    {IDB_onoffgreen,    onoffgreen},
    {IDB_dbblack,       dbblack},
    {IDB_panelfx,       panelfx},
    {IDB_panelvoice,    panelvoice},
    {IDB_vu,            vu},
    {0, 0}
};
#endif
...
extern char	*note2str(long note);

void convert_bendrange (float value, char*string)
{
	sprintf(string," %2d ",int(value*12));
}

void convert_tone (float value, char*string)
{
	sprintf (string, "%4.0f", 300+3500*value);
}

void convert_speed (float value, char*string)
{
...
void convert_perc (float value, char*string)
{
...
void convert_shape (float value, char*string)
{
	sprintf(string,"  %d ",int(value*(W_NUMOF-1))+1);
}
...
void Editor::idle()
{
	...
	if(!justloaded && getTicks()-oldTicks<200)
		return;
	if(!controls[n_1_midi] || !controls[n_1_midi] || !controls[n_1_midi]
		|| !ctrl_display)
		return;
	...
	if(justloaded && ctrl_display)
	{
		if(ticks<100)
		{
			...
			ctrl_display->setstring(1,"=== Rumpelrausch T�ps ===");
...
// initialize control array
	...
	rct(387,17,529,68);
	ctrl_display=new CDisplay(rct,this,n_display,bitmaps[b_panelvoice]);
...
//-------- enumerated controls
	...
	ctrl->setStringConvert(convert_bendrange);
	frame->addView(controls[c]);
	{
		CCircDisplay*	ctrl1=(CCircDisplay*) controls[c];
		ctrl1->setStringConvert(convert_bendrange);
	}
	ctyp[c]=cCCircDisplay;
	...
	controls[c]=new CCircDisplay(rct,this,
		c,bitmaps[b_cknob],bitmaps[b_yellow],bitmaps[b_panelfx]);
	ctrl=(CCircDisplay*)controls[c];
	ctrl->setStringConvert(convert_shape);
	frame->addView(controls[c]);
	ctyp[c]=cCCircDisplay;
	...
	controls[c]=new CCircDisplay(rct,this,
		c,bitmaps[b_cknob],bitmaps[b_yellow],bitmaps[b_panelfx]);
	ctrl=(CCircDisplay*)controls[c];
	ctrl->setStringConvert(convert_perc);
	frame->addView(controls[c]);
	ctyp[c]=cCCircDisplay;
	...
	controls[c]=new CFader(rct,this,
...
//------ FX params ---------------------------------------
	...
	controls[c]=new CCircDisplay(rct,this,
		c,bitmaps[b_cknob],bitmaps[b_yellow],bitmaps[b_panelfx]);
	ctrl=(CCircDisplay*)controls[c];
	ctrl->setStringConvert(convert_tone);
	ctyp[c]=cCCircDisplay;
	...
	controls[c]=new CCircDisplay(rct,this,
		c,bitmaps[b_cknob],bitmaps[b_yellow],bitmaps[b_panelfx]);
	ctrl=(CCircDisplay*)controls[c];
	ctrl->setStringConvert(convert_speed);
	ctyp[c]=cCCircDisplay;
...
//------ splash screen ---------------------------------------
	...
	frame->addView (controls[n_mrvalve]);
	frame->addView (controls[n_drive]);
	frame->addView (controls[n_set]);
	frame->addView (controls[n_tone]);
	frame->addView (controls[n_mix]);
	frame->addView (controls[n_speakers]);
	frame->addView (controls[n_speed]);
	frame->addView (controls[n_l_slow]);
	frame->addView (controls[n_l_fast]);
	frame->addView (controls[n_u_slow]);
	frame->addView (controls[n_u_fast]);
	frame->addView (controls[n_belt]);
	frame->addView (controls[n_spread]);
	frame->addView (controls[n_complex]);
	frame->addView (controls[n_pedalspeed]);
...
void Editor::setParameter  (long index, float value)
{
	...
	if (index == 1000)			// mouse button down on control
	{
		int	tag=(int)value;
		if(value==-1)
		{
		...
		frame->invalidate (ctrl_display->getViewSize());
	}
	...
	{
		char txt[64];
		sprintf(txt,"Splitpoint=%s",note2str(long(value*128)));
	...
	else if((index>=0 && index<kNumParams) || index==n_compare)
	{
		...
		if(ctrl!=NULL)
		{
			...
			ctrl->setValue(value);
			ctrl->setDirty(true);
    		frame->invalidate (ctrl->getViewSize());
    		frame->invalidate (ctrl_display->getViewSize());
    	}
	}
