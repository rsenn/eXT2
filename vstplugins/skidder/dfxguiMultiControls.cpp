...
//------------------------------------------------------------------------
void XYbox::mouse(CDrawContext *pContext, CPoint &where, long button)
{
	mouseDown = true;
...
//	COffscreenContext *pOScreen = new COffscreenContext(getParent(), controlWidth, controlHeight, kBlackCColor);
    ...
    pContext->offset (size.left, size.top);
	...
	{
		if (bTransparencyEnabled)
			pBackground->drawTransparent(pContext, rect, offset);
		else
			pBackground->draw(pContext, rect, offset);
	}
	...
	{
		if (bDrawTransparentEnabled)
			pHandle->drawTransparent(pContext, rect1, where);
		else
			pHandle->draw(pContext, rect1, where);
	}
	if (pHandle2)
	{
		if (bDrawTransparentEnabled)
			pHandle2->drawTransparent(pContext, rect2);
		else
			pHandle2->draw(pContext, rect2);
	}
...
//------------------------------------------------------------------------
void CHorizontalRangeSlider::mouse(CDrawContext *pContext, CPoint &where, long button)
{
	if (!bMouseEnabled)
		return;
...
//------------------------------------------------------------------------
void CVerticalRangeSlider::mouse(CDrawContext *pContext, CPoint &where, long button)
{
	if (!bMouseEnabled)
		return;
