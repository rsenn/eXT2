...
void OPMDRV::SendKc(int ch){
 ...
 Note=TblCh[ch].Note;
 if(Note<0x80){
	Word=(Note<<16)+((TblMidiCh[MidiCh].Bend<<3)*TblMidiCh[MidiCh].BendRang)+TblCh[ch].PortaBend;	//16.16bit
	Note=Word>>16;
//	kc=(Ktbl[(Note-3)%12])+((int)((Note-3)/12)*16); //Kc[Oct::key]

	 kc=(Ktbl[(Note-1)%12])+((int)((Note-1)/12)*16); //Kc[Oct::key]
	kf=(Word>>8)&0xfc;							 //Kf
	_iocs_opmset(0x28+ch,kc);	// 
	_iocs_opmset(0x30+ch,kf);	//
 }
 #ifdef MyDEBUG
...
class OPDATA *pOp;
	...
	for(i=0;i<4;i++){
		...
		_iocs_opmset(0x80+tmp,(pOp->KS<<5)|(pOp->AR&0x1f));	 //KS/AR
...
//
	...
	if(TblMidiCh[MidiCh].LFOdelay==0){
		...
		_iocs_opmset(0x38+ch,0);//PMS/AMS LFO
	}
...
//Vel&Vol Set
	_iocs_opmset(0x02,TblMidiCh[TblCh[ch].MidiCh].Vol);			
	_iocs_opmset(0x04,TblCh[ch].Vel);							

	//_iocs_opmset(0x01, 2);	
 if(PolyMono==false && BfNote!=0xff){
	}else{
		_iocs_opmset(0x01, 0);	
		_iocs_opmset(0x08,(ch&0x07)+(VoTbl[TblMidiCh[MidiCh].VoNum].OpMsk&0x78));
	}
...
void OPMDRV::VoCng(int MidiCh,int Vo){
	...
	Amd=CalcLim8(VoTbl[Vo].AMD,TblMidiCh[MidiCh].AMD);
	Pmd=CalcLim8(VoTbl[Vo].PMD,TblMidiCh[MidiCh].PMD);
	...
	while(fgets(line,256,File)!=NULL){
		...
		while(*pline!=0 && *pline!=0x0a){				
			...
			if(strncmp("@:",pline,2)==0){
				...
				errMsk|=0x01;
				while(*pline!=0 && *pline!=0x0a){pline++;}
			}
...
int OPMDRV::CalcLim8(int VoData,int DifData){
	int ret;
	ret=VoData+(((DifData&0x7fff)-0x4000)>>6);
